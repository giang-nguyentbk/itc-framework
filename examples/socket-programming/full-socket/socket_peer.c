...
int main(int argc, char* argv[])
{
	...
	while(!is_terminated)
	{
		...
		for(int i = 0; i < MAX_PEERS; i++)
		{
			if(m_inst.peers[i].tcp_sending_fd > 0)
			{
				printf("\tERROR: \"%s:%d\" main - DEBUG xxx %d!\n", __FILE__, __LINE__, FD_ISSET(m_inst.peers[i].tcp_sending_fd, &fdset));
			}
...
static bool handle_receive_greeting_msg(int sockfd)
{
	...
	if(iter != NULL && (*iter)->tcp_sending_fd != INVALID_FD)
	{
		printf("\tABN: \"%s:%d\" handle_receive_greeting_msg - Pre-iter %d!\n", __FILE__, __LINE__, (*iter)->tcp_listening_fd);
	}
	...
	} else if(iter != NULL && (*iter)->once == false)
	{
		...
		char temp_msg[255];
		strcpy(temp_msg, "Hello my TCP peer 3333...");
	...
	} else
	{
		...
		for(; i < MAX_PEERS; i++)
		{
			if(m_inst.peers[i].state == PEER_DISCONNECTED)
			{
				...
				char temp_msg[255];
				strcpy(temp_msg, "Hello my TCP peer 4444...");
...
static bool handle_accept_incoming_connection(int sockfd)
{
	...
	peer.tcp_listening_addr.sin_port = TCP_LISTENING_PORT;
