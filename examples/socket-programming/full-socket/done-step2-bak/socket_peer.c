...
Connect sequence:
	Peer A: Broadcasting greeting messages					: No peer in the A's list
	Peer B: Received above message						: Add peer A to the B list with PEER_CONNECTING state
	Peer B: connect() which triggers peer A accept()			: Add peer B to the A list with PEER_CONNECTING state
	Peer B: Send get_namespace request along with B's namespace		: Peer A state in B list is still PEER_CONNECTING until peer B receiving get_namespace reply
	Peer A: Received above request						: Set peer B state in A list to PEER_CONNECTED
	Peer A: Send get_namespace reply					:
	Peer B: Received above reply						: Set peer A state in B list to PEER_CONNECTED
...
int main(int argc, char* argv[])
{
	...
	while(!is_terminated)
	{
		...
		printf("\tINFO: \"%s:%d\" main - Resetting loop after %d seconds!\n", __FILE__, __LINE__, BROADCAST_INTERVAL);
	}
...
static bool setup_my_TCP(void)
{
	m_inst.my_TCP_fd = socket(AF_INET, SOCK_STREAM, 0);
	if(m_inst.my_TCP_fd < 0)
	{
	...
	int listening_opt = 1;
	int res = setsockopt(m_inst.my_TCP_fd, SOL_SOCKET, SO_REUSEADDR, &listening_opt, sizeof(int));
	if(res < 0)
	{
	...
	m_inst.my_TCP_addr.sin_addr = get_ip_address_from_network_interface(m_inst.my_TCP_fd, NETWORK_INTERFACE_ETH0);
	m_inst.my_TCP_addr.sin_port = htons(TCP_LISTENING_PORT2);

	res = bind(m_inst.my_TCP_fd, (struct sockaddr *)&m_inst.my_TCP_addr, size);
	if(res < 0)
	{
