...
#define ITC_GATEWAY_MBOX_UDP_NAME2	"itc_gw_udp_mailbox2"
#define ITC_GATEWAY_MBOX_TCP_SER_NAME2	"itc_gw_tcp_server_mailbox2"
...
static bool setup_udp_peer(void)
{
	...
	printf("\tINFO: setup_peer_UDP - Setup peer UDP successfully on %s:%d\n", inet_ntoa(itcgw_inst.udp_peer_addr.sin_addr), ITC_GATEWAY_BROADCAST_PORT); // TEST ONLY
	return true;
}
...
static bool setup_tcp_server(void)
{
	...
	itcgw_inst.tcp_server_addr.sin_port = htons(TCP_LISTENING_PORT2);
...
static bool setup_tcp_server_mailbox(void)
{
	itcgw_inst.tcp_server_mbox_id = itc_create_mailbox(ITC_GATEWAY_MBOX_TCP_SER_NAME2, ITC_NO_NAMESPACE);
	if(itcgw_inst.tcp_server_mbox_id == ITC_NO_MBOX_ID)
	{
		printf("\tERROR: setup_tcp_server_mailbox - Failed to create mailbox %s\n", ITC_GATEWAY_MBOX_TCP_SER_NAME2);
		return false;
	}
...
static bool setup_tcp_client_mailbox(void)
{
	itcgw_inst.tcp_client_mbox_id = itc_create_mailbox(ITC_GATEWAY_MBOX_TCP_CLI_NAME2, ITC_NO_NAMESPACE);
